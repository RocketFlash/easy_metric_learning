...
def main(CONFIGS, source, window_size=20, step_size=10, is_horizontal=True):
    ...
    for im_i in tqdm(range(len(images_paths)-1)):
        ...
        if is_horizontal:
            if im_i==0:
                ...
                stitched_image = cv2.resize(stitched_image, (st_w, target_image_small.shape[0]))
                stitched_image = np.concatenate([target_image_small[:, :target_coords[0][0],:], stitched_image[:, source_coords[0][0]:, :]], axis=1)
        else:
            if im_i==0:
                stitched_image = np.concatenate([target_image_small[:target_coords[0][1], :,:], source_image_small[source_coords[0][1]:, :, :]], axis=0)
            else:
                st_h, st_w, st_c = stitched_image.shape
                stitched_image = cv2.resize(stitched_image, (target_image_small.shape[1], st_h))
                stitched_image = np.concatenate([target_image_small[:target_coords[0][1], :,:], stitched_image[source_coords[0][1]:, :, :]], axis=0)
    ...
    cv2.imwrite(str(CONFIGS["MISC"]["TMP"]/f'stitched.png'), stitched_image)

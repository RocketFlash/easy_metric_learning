...
def parse_args():
    ...
    parser.add_argument(
        "--dataset_info", default="", help="path to the csv file describing dataset"
    )
...
if __name__ == "__main__":
    ...
    df_folds_train.to_csv(
        dataset_path / f"{args.save_name}_train_only.csv", index=False
    )
    df_folds_test.to_csv(dataset_path / f"{args.save_name}_test_only.csv", index=False)

    df_kfold = get_stratified_kfold(df, k=args.k, random_state=args.random_seed)

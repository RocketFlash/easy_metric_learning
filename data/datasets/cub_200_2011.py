...
class CUB(BaseDataset):
    ...
    def download(self):
        if not self.dataset_folder.is_dir():
            ...
            shutil.move(
                self.dataset_path / "attributes.txt", ds_path / "attributes.txt"
            )
    ...
    def prepare(self):
        df_images = pd.read_csv(
            self.dataset_folder / "images.txt", sep=" ", names=["id", "file_name"]
        )
        df_bboxes = pd.read_csv(
            self.dataset_folder / "bounding_boxes.txt",
            sep=" ",
            names=["id", "x1", "y1", "w", "h"],
            dtype={"id": int, "x1": int, "y1": int, "w": int, "h": int},
        )
        ...
        df_classes = pd.read_csv(
            self.dataset_folder / "classes.txt", sep=" ", names=["class_id", "label"]
        )
        df_img_to_class = pd.read_csv(
            self.dataset_folder / "image_class_labels.txt",
            sep=" ",
            names=["id", "class_id"],
        )
        df_img_to_class = df_img_to_class.assign(
            is_test=[1 if x > 100 else 0 for x in df_img_to_class["class_id"]]
        )

        df_img_to_class["label"] = df_img_to_class["class_id"].map(
            dict(df_classes.values)
        )
        ...
        df_info = df_info[["file_name", "label", "width", "height", "bbox", "is_test"]]

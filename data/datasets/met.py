...
class MET(BaseDataset):
    ...
    def prepare(self):
        ...
        test_labels = [
            [anno["path"], anno["MET_id"]] for anno in test_anno if "MET_id" in anno
        ]
        val_labels = [
            [anno["path"], anno["MET_id"]] for anno in val_anno if "MET_id" in anno
        ]

        df_test = pd.DataFrame(test_labels, columns=["file_name", "label"])
        df_val = pd.DataFrame(val_labels, columns=["file_name", "label"])
        df_test["is_test"] = 1
        df_val["is_test"] = 0
        ...
        for class_folder_path in class_folders_path:
            images = sorted(
                [
                    l
                    for l in list(class_folder_path.glob("*.jpeg"))

...
class Inshop(BaseDataset):
    ...
    def download(self):
        if not self.dataset_folder.is_dir():
            ...
            gdown.download(id=self.images_url, output=str(z_file_path), quiet=False)
            ...
            gdown.download(id=self.split_url, output=str(split_file_path), quiet=False)
            print(f"Dataset have been downloaded and extracted")

    def prepare(self):
        df_info = pd.read_csv(
            self.dataset_folder / "list_eval_partition.txt", sep="\s+", skiprows=1
        )
        df_info = df_info.rename(
            columns={"image_name": "file_name", "item_id": "label"}
        )
        df_info = df_info.assign(
            is_test=[0 if x == "train" else 1 for x in df_info["evaluation_status"]]
        )
        df_info = self.add_image_sizes(df_info, self.dataset_folder)

        df_info["label"] = df_info["label"].apply(lambda x: f"inshop_{x}")
        df_info = df_info[
            ["file_name", "label", "width", "height", "evaluation_status", "is_test"]
        ]

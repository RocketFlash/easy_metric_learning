...
def generate_images_grid(
    class_name, similarity_dict, dataset_path, n_samples=1, save_path="./"
):
    n_sim = len(similarity_dict)

    fig = plt.figure(figsize=(15, 15))
    grid = ImageGrid(fig, 111, nrows_ncols=(n_samples, n_sim + 1), axes_pad=0.2)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--result_csv", type=str, default="", help="path to references csv"
    )
    parser.add_argument(
        "--dataset_path", type=str, default="", help="path to references csv"
    )
    parser.add_argument(
        "--save_name", type=str, default="similar_stat", help="name of saved file"
    )
    parser.add_argument(
        "--min_similarity", type=float, default=0.5, help="path to references csv"
    )
    ...
    if not SAVE_FILE_NAME.is_file():
        df = pd.read_csv(
            CSV_PATH, dtype={"file_name": str, "gt": str, "prediction": str}
        )
        ...
        for class_name, class_group in tqdm(classes_groups):
            for index, row in class_group.iterrows():
                ...
                similar_classes_dict[class_name] = (
                    similar_classes_dict.get(class_name, []) + similar_classes
                )
        ...
        for class_name, similar_classes in similar_classes_dict.items():
            ...
            if similar_classes_counter:
                similar_classes_counter = {
                    k: float(v) / n_predictions
                    for k, v in similar_classes_counter.items()
                }
    ...
    for class_name, class_sim_stat in similar_classes_stat.items():
        class_sim_stat = dict(
            filter(lambda item: item[1] >= MIN_SIMILARITY, class_sim_stat.items())
        )
        if class_sim_stat:
    ...
    for class_name, similarity_dict in tqdm(bidirectional_connections.items()):
        generate_images_grid(
            class_name, similarity_dict, DATASET_PATH, n_samples=2, save_path=SAVE_PATH
        )

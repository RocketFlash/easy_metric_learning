...
def parse_args():
    ...
    parser.add_argument(
        "--descriptions_path", default="", help="path to the csv descriptions folder"
    )
...
if __name__ == "__main__":
    ...
    for desc_file in tqdm(desc_files):
        ...
        df_descriptions = pd.read_table(desc_file, delimiter=",", dtype=str)

        df_info = df_descriptions[
            ["name", "category_name", "primary_code", "primary_image"]
        ]
    ...
    if args.filter:
        df_info = df_combined[["upc", "retailer", "name", "category"]]
        ...
        df_dedup = pd.DataFrame(
            dedup_rows, columns=["upc", "name", "category", "retailer"], dtype=str
        )

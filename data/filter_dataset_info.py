...
def parse_args():
    ...
    parser.add_argument(
        "--threshold", type=int, default=10, help="threshold for duplicates selection"
    )
...
def duplicates_removal_filtering(df):
    ...
    for label, label_group in tqdm(labels_groups):
        classes_dict[label] = pd.Series(
            label_group.hash.values, index=label_group.file_name
        ).to_dict()

    progress_bar = tqdm(classes_dict.items(), total=len(classes_dict))

    for cl_name, cl_imgs in progress_bar:
        duplicates = phasher.find_duplicates_to_remove(
            encoding_map=cl_imgs, max_distance_threshold=THRESHOLD
        )

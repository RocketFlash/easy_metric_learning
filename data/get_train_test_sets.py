...
def parse_args():
    parser = argparse.ArgumentParser(
        description="split dataset on training and testing parts"
    )
    parser.add_argument(
        "--dataset_info", default="./", help="path to the dataset info file"
    )
    parser.add_argument(
        "--split_type",
        type=str,
    ...
    parser.add_argument("--random_seed", type=int, default=28, help="random seed")
    return parser.parse_args()


def split_by_frequency_bins(
    counts_df, test_ratio=0.1, min_frequency=10, random_seed=28
):
    ...
    for bin in n_samples_bins:
        ...
        train_bin_df.reset_index(inplace=True, drop=True)

        test_bin_df.reset_index(inplace=True, drop=True)
...
if __name__ == "__main__":
    ...
    dataset_train = df[df["label"].isin(train_classes_df["label"])].reset_index(
        drop=True
    )

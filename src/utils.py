...
def _get_module_class_names(model):
    module_classes = {"<root>": model.__class__.__name__}
...
def _get_legacy_checkpoint_safe_globals():
    ...
    try:
        ...
        safe_globals.append(SimpleNamespace)
    except ImportError:
        pass
...
def _load_weights_only_checkpoint(ckp_path, map_location=None):
    try:
        return torch.load(ckp_path, map_location=map_location, weights_only=True)
    except pickle.UnpicklingError as exc:
        safe_globals_ctx = getattr(torch.serialization, "safe_globals", None)
        if safe_globals_ctx is None:
            raise

        with safe_globals_ctx(_get_legacy_checkpoint_safe_globals()):
            checkpoint = torch.load(
                ckp_path, map_location=map_location, weights_only=True
            )

        warnings.warn(
            (
                f"Loaded legacy checkpoint {ckp_path} with an explicit safe_globals allowlist. "
                "Regenerate the checkpoint with the current code to remove this compatibility path."
            ),
            RuntimeWarning,
        )
        return checkpoint
...
def save_ckp(
    ...
    if accelerator is not None:
        ...
        if emb_model_only and hasattr(model_, "embeddings_net"):
            model_for_checkpoint = model_.embeddings_net
        else:
            model_for_checkpoint = model_
    else:
        if emb_model_only and hasattr(model, "embeddings_net"):
            model_for_checkpoint = model.embeddings_net
        else:
    ...
    checkpoint = {
        ...
        "model_module_classes": _get_module_class_names(model_for_checkpoint),
    }

    if not emb_model_only:
        if best_criterion_val is not None:
            best_criterion_val = float(best_criterion_val)
        checkpoint.update(
            {
                "epoch": epoch,
...
def load_ckp(
    ...
    if accelerator is not None:
        checkpoint = _load_weights_only_checkpoint(
            ckp_path, map_location=accelerator.device
        )
    else:
        checkpoint = _load_weights_only_checkpoint(ckp_path, map_location=device)
    ...
    unexpected_keys = []
    shape_mismatches = []
    module_class_mismatches = []

    if strict and "model_module_classes" in checkpoint:
    ...
    missing_keys = [key for key in model_state_dict if key not in compatible_dict]
    if strict and (
        unexpected_keys or missing_keys or shape_mismatches or module_class_mismatches
    ):
        details = []
        if module_class_mismatches:
            details.append(f"module class mismatches: {module_class_mismatches[:10]}")
        if unexpected_keys:
            details.append(f"unexpected keys: {unexpected_keys[:10]}")
        if missing_keys:
            details.append(f"missing keys: {missing_keys[:10]}")
        if shape_mismatches:
            details.append(f"shape mismatches: {shape_mismatches[:10]}")
        raise RuntimeError(
            f'Checkpoint {ckp_path} does not match model state: {"; ".join(details)}'
        )

    if not compatible_dict:
        raise RuntimeError(
            f"Checkpoint {ckp_path} has no weights compatible with the target model"
        )

    if not strict and (unexpected_keys or missing_keys or shape_mismatches):
        warnings.warn(
            (
                f"Partially loading checkpoint {ckp_path}: "
                f"{len(compatible_dict)} loaded, "
    ...
    model_state_dict.update(compatible_dict)
    model.load_state_dict(model_state_dict)

    if optimizer is not None:
        if "optimizer" not in checkpoint:
            warnings.warn(
                f"Checkpoint {ckp_path} has no optimizer state", RuntimeWarning
            )
        else:
    ...
    best_criterion_val = (
        checkpoint["best_criterion_val"] if "best_criterion_val" in checkpoint else None
    )

    return model, optimizer, epoch, best_criterion_val


def load_config(config_path, resolve=True):
    """Load an OmegaConf config and return it as a plain container."""
    config = OmegaConf.load(config_path)
    return OmegaConf.to_container(config, resolve=resolve)


def get_images_paths(path, extensions=IMAGE_EXTENSIONS):
    path = Path(path)
    return sorted(
...
def load_checkpoint(
    ...
    if mode == "resume":
        ...
        model, optimizer, epoch, best_criterion_val = load_ckp(
            ckp_path, model, optimizer, device=device, accelerator=accelerator
        )
        ...
        if hasattr(model, "embeddings_net"):
            ...
            model, _, _, _ = load_ckp(
                ckp_path, model, device=device, accelerator=accelerator, strict=False
            )
        checkpoint_data["model"] = model
        ...
        model, _, _, _ = load_ckp(
            ckp_path, model, device=device, accelerator=accelerator
        )
        checkpoint_data["model"] = model
    else:
        if logger is not None:
...
def load_model_except_torch(weights, model_type="torch", device="cpu", logger=None):

    model_info = {"model_type": model_type}

    if model_type == "traced":
        if logger is not None:
            logger.info(f"load traced model from: {weights}")
        ...
        if logger is not None:
            logger.info(f"load onnx model from: {weights}")
        ...
        if logger is not None:
            logger.info(f"load tensorflow model from: {weights}")
...
def plot_embeddings_interactive(
    ...
    if method == "fast_tsne":
        ...
        tsne_train = TSNE(
            random_state=random_state, n_jobs=n_jobs, n_components=n_components
        ).fit(embeddings)
    elif method == "umap":
        from umap import UMAP

        umap = UMAP(
            n_jobs=n_jobs, n_components=n_components, init="random", random_state=28
        )
    ...
    fig.update_layout(
        title=go.layout.Title(text=f"{method} plot", xref="paper", x=0),
        autosize=False,
        width=1000,
        height=1000,
        showlegend=False,

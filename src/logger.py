...
class Logger:
    def __init__(self, path=None, accelerator=None):
    ...
    def info_config(self, config):
        ...
        self.info(json.dumps(config_dict, sort_keys=False, indent=4))

    def info_data(self, dataset_stats):
    ...
    def info_model(
        ...
        model_info_str += (
            f'{self.tab_string*2}{"Backbone":<15}: {config.backbone.type}\n'
        )
        model_info_str += f'{self.tab_string*2}{"Head":<15}: {config.head.type}\n'
        if "margin" in config:
            model_info_str += (
                f'{self.tab_string*2}{"Margin":<15}: {config.margin.type}\n'
            )
        model_info_str += (
            f'{self.tab_string*2}{"Embeddings size":<15}: {config.embeddings_size}\n'
        )
        ...
        if stats_valid is not None:
            ...
            if "metrics" in stats_valid:
                ...
                for k_metric, v_metric in stats_valid["metrics"].items():
                    epoch_info_str += (
                        f"{self.tab_string*2}{k_metric:<15} : {v_metric:.5f}\n"
                    )

...
class BaseEvaluator:
    def __init__(
        ...
        if is_eval:
            ...
            self.knn_algo = get_knn_search(
                config.evaluation.knn, K=self.K, save_dir=save_dir
            )
        else:
    ...
    def generate_embeddings(self, data_info, n_batches=None):
        if self.model_info is not None:
        ...
        tqdm_test = tqdm(data_info.dataloader, total=total_)
        index = 0
        ...
        with torch.no_grad():
            for batch_index, (images, targets, fnames) in enumerate(tqdm_test):
                ...
                if self.model_info is not None:
                    if self.model_info["model_type"] in ["torch", "traced"]:
                    ...
                    elif self.model_info["model_type"] in [
                        "tf_32",
                        "tf_16",
                        "tf_int",
                        "tf_dyn",
                        ...
                        data_np = torch.permute(images, (0, 2, 3, 1)).numpy()
                        input_scale, input_zero_point = self.model_info[
                            "input_details"
                        ]["quantization"]
                        output_scale, output_zero_point = self.model_info[
                            "output_details"
                        ]["quantization"]
                        tf_sample_int8 = data_np / input_scale + input_zero_point
                        tf_sample_int8 = tf_sample_int8.astype(
                            self.model_info["input_details"]["dtype"]
                        )
    ...
    def calculate_metrics(self, df_nearest, dataset_name="dataset"):
        predictions = np.array(
            [row.astype(str) for row in df_nearest["prediction"].to_numpy()]
        )
        ...
        if self.save_results:
            df_metrics.to_csv(
                self.save_dir / f"{dataset_name}_metrics.csv", index=False
            )

...
def get_model(config_backbone, config_head, config_margin=None, n_classes=1000):

    embeddings_model = get_model_embeddings(
        config_backbone=config_backbone, config_head=config_head
    )
    ...
    if config_margin is not None:
        ...
        model = MLNet(embeddings_net=embeddings_model, margin=margin)
    else:
...
def get_model_teacher(config_teacher, device=None, accelerator=None):
...
def load_emb_model_and_weights(
    config_backbone, config_head, weights=None, device=None, accelerator=None
):
    model = get_model(
    ...
    if weights is not None:
        checkpoint_data = load_checkpoint(
            weights, model=model, mode="emb", device=device, accelerator=accelerator
        )

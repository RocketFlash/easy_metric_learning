...
def get_train_data_from_config(config, logger=None):
    ...
    data_infos = edict(
        dict(train=data_info_train, valid=data_info_valid, dataset_stats=dataset_stats)
    )
...
def get_test_data_from_config(config, logger=None):
    ...
    for dataset_config in config.evaluation.data.datasets:
        ...
        df_test = get_test_split(annotation=annotations, fold=fold)
        labels_to_ids_path = (
            dataset_config.labels_to_ids_path
            if "labels_to_ids_path" in dataset_config
            else None
        )
...
def get_loader(
    ...
    dataset = get_dataset(root_dir, df_names, transform, labels_to_ids, dataset_config)

...
def get_images_paths(path):
    ...
    return [
        l
        for l in list(pathlib_path.glob("**/*.jpeg"))
...
def generate_representative_dataset(
    images_path, image_size=(224, 224), n_max=200, save_path="./weights/"
):
...
def test_model(model, sample, to_profile=False, n_times=10, model_type="torch"):
    if to_profile:
    ...
    with torch.no_grad():
        ...
        for i in range(n_times):
            ...
            if model_type == "onnx":
                model_output = model.run(None, {"input": sample.cpu().numpy()})

...
from converter.onnx import convert_onnx, simplify_onnx
...
def parse_args():
    ...
    parser.add_argument(
        "--dataset_path", default="", help="path to the calibration dataset"
    )
    ...
    parser.add_argument(
        "--q_dynamic", action="store_true", help="Use dynamic quantization"
    )
    parser.add_argument("--profile", action="store_true", help="profile model")
    parser.add_argument(
        "--mobile", action="store_true", help="convert model using mobile optimizers"
    )
    parser.add_argument(
        "--metal", action="store_true", help="convert model to metal backend"
    )
    parser.add_argument("--nnapi", action="store_true", help="convert model to nnapi")
    parser.add_argument(
        "--vulkan", action="store_true", help="convert model to vulkan backend"
    )
    ...
    parser.add_argument(
        "--tf_q", action="store_true", help="do quantization for tensorflow model"
    )
...
if __name__ == "__main__":
    ...
    if args.dataset_path:
        ...
        sample = get_sample(str(image_paths[0]), transform=transform_test)
        sample_test = get_sample(str(image_paths[1]), transform=transform_test)
    else:
    ...
    if not args.no_trace:
        test_model(model, sample_test, to_profile=args.profile, n_times=n_times)
        ...
        test_model(traced_model, sample_test, to_profile=args.profile, n_times=n_times)
    ...
    if args.q:
        ...
        test_model(model_q, sample_test, to_profile=args.profile, n_times=n_times)
        ...
        test_model(
            traced_q_model, sample_test, to_profile=args.profile, n_times=n_times
        )
    else:
        model_q = model
    ...
    if args.onnx or args.tf:
        ...
        model_onnx_non_sim = convert_onnx(model, sample, save_path, model_name)
        model_onnx = simplify_onnx(model_onnx_non_sim)
        onnx.save(model_onnx, onnx_simp_save_name)

        ort_session = ort.InferenceSession(
            onnx_simp_save_name, providers=["CPUExecutionProvider"]
        )

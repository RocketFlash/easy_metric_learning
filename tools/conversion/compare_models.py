...
from src.utils import get_device, get_images_paths, get_sample
...
def parse_args():
    ...
    parser.add_argument(
        "--device", default="cpu", type=str, help="device for calculations"
    )
...
if __name__ == "__main__":
    ...
    for config_name in (
        ...
        config = args.work_folder / config_name
        if config.is_file():
            break
    weights = args.work_folder / "best_emb.pt"
    if not weights.is_file():
        weights = args.work_folder / "last_emb.pt"
    WEIGHTS_PATH = args.work_folder / "weights"

    device = get_device(args.device)
    assert os.path.isfile(config)
    CONFIGS = OmegaConf.load(config)
    img_size = CONFIGS.img_h
    model_name = CONFIGS.run_name
    transform_test = get_transform(CONFIGS.transform.test)
    ...
    WEIGHTS_TF_DYNAMIC = str(
        WEIGHTS_PATH_TF / f"{model_name}_simp_dynamic_range_quant.tflite"
    )
    WEIGHTS_TF_INT = str(WEIGHTS_PATH_TF / f"{model_name}_simp_integer_quant.tflite")
    WEIGHTS_TF_FULL_INT = str(
        WEIGHTS_PATH_TF / f"{model_name}_simp_full_integer_quant.tflite"
    )
    ...
    model = load_emb_model_and_weights(
        config_backbone=CONFIGS.backbone,
        config_head=CONFIGS.head,
        weights=weights,
        device=device,
    )
    ...
    ort_session = ort.InferenceSession(
        WEIGHTS_ONNX, providers=["CUDAExecutionProvider", "CPUExecutionProvider"]
    )
    ...
    if args.dataset_path:
        ...
        for image_path in tqdm(images_paths):
            sample = get_sample(str(image_path), transform=transform_test)
            ...
            with torch.no_grad():
                ...
                o_tf_lite_fint = output_scale * (
                    o_tf_lite_fint["output"].astype(np.float32) - output_zero_point
                )

                cos_sim_traced_i = cosine_similarity(o_orig, o_traced)
                cos_sim_onnx_i = cosine_similarity(o_orig, torch.from_numpy(o_onnx[0]))
                cos_sim_tf32_i = cosine_similarity(
                    o_orig, torch.from_numpy(o_tf_lite_f32["output"])
                )
                cos_sim_tf16_i = cosine_similarity(
                    o_orig, torch.from_numpy(o_tf_lite_f16["output"])
                )
                cos_sim_dyn_i = cosine_similarity(
                    o_orig, torch.from_numpy(o_tf_lite_dyn["output"])
                )
                cos_sim_int_i = cosine_similarity(
                    o_orig, torch.from_numpy(o_tf_lite_int["output"])
                )
                cos_sim_fint_i = cosine_similarity(
                    o_orig, torch.from_numpy(o_tf_lite_fint)
                )
    ...
    if args.image_path:
        sample = get_sample(args.image_path, transform=transform_test)
        with torch.no_grad():
            sample_np = torch.permute(sample, (0, 2, 3, 1)).numpy()
            tf_sample = tf.convert_to_tensor(sample_np)

...
def parse_args():
    ...
    parser.add_argument(
        "--ref_csv", type=str, default="", help="path to references csv"
    )
    parser.add_argument("--test_csv", type=str, default="", help="path to test csv")
    parser.add_argument(
        "--top_n", type=int, default=5, help="number of nearest neighbors"
    )
    ...
    parser.add_argument(
        "--filter_labels",
        action="store_true",
        help="test samples only on labels presented in test set",
    )
    ...
    parser.add_argument(
        "--centroids",
        action="store_true",
        help="calculate centroids for gallery images",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="path to embeddings if train and test models are different",
    )
...
if __name__ == "__main__":
    ...
    if args.embeddings_test:
        embeddings_test, labels_test, file_names_test, eval_status_test = (
            read_embeddings(args.embeddings_test)
        )

    if args.ref_csv:
        df = pd.read_csv(
            args.ref_csv,
            dtype={"label": str, "file_name": str, "width": int, "height": int},
        )
    ...
    if train_df is not None:
        ...
        train_embeddings, train_labels, train_fnames = filter_embeddings(
            embeddings_dict, train_df, centorids=args.centroids
        )
        print("N gallery embeddings:", train_embeddings.shape)
        if embeddings_test is not None:
            embeddings_test_dict = get_embeddings_dict(
                embeddings_test, labels_test, file_names_test
            )
            test_embeddings, test_labels, test_fnames = filter_embeddings(
                embeddings_test_dict, test_df
            )
        else:
            test_embeddings, test_labels, test_fnames = filter_embeddings(
                embeddings_dict, test_df
            )
    else:
        if eval_status.shape:
            ...
            train_embeddings, train_labels, train_fnames = (
                embeddings[mask_gallery],
                labels[mask_gallery],
                file_names[mask_gallery],
            )
            if embeddings_test is not None:
                test_embeddings, test_labels, test_fnames = (
                    embeddings_test[mask_query],
                    labels_test[mask_query],
                    file_names_test[mask_query],
                )
            else:
                test_embeddings, test_labels, test_fnames = (
                    embeddings[mask_query],
                    labels[mask_query],
                    file_names[mask_query],
                )
        else:
            if args.verbose:

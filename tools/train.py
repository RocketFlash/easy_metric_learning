...
def train(config):
    ...
    logger = Logger(work_dir / "log.txt", accelerator=accelerator)
    ...
    best_criterion_val = (
        10000 if config.train.best_model_criterion.type == "loss" else -10000
    )
    start_epoch = 1
    ...
    if config.load_checkpoint is not None:
        ...
        start_epoch = (
            checkpoint_data["start_epoch"]
            if "start_epoch" in checkpoint_data
            else start_epoch
        )
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        if is_main_process(accelerator):
            ...
            if is_best:
                ...
                shutil.copyfile(
                    save_paths.last_weights_path, save_paths.best_weights_path
                )
                shutil.copyfile(
            ...
            logger.info_epoch_train(epoch, stats_train, stats_valid)

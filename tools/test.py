...
def test(config):
    ...
    if config.model_type == "torch":
        ...
        model_info = load_model_except_torch(
            config.weights, model_type=config.model_type, device=device, logger=logger
        )

    eval_save_dir = work_dir / "eval_results"
    evaluator = get_evaluator(
        config, save_dir=eval_save_dir, device=device, model_info=model_info, pca=pca
    )
    ...
    all_metrics_df.to_csv(
        work_dir / f"eval_result_on_{config.evaluation.data.name}.csv"
    )

...
from src.pca import save_pca


@hydra.main(version_base=None, config_path="../../configs/", config_name="config_pca")
def test(config):
    ...
    if not embeddings_path.is_file():
        if config.model_type == "torch":
            ...
            if config.head.type == "no_head":
                ...
                logger.info(
                    f"Embeddings size changed to backbone output size {config.embeddings_size}"
                )
            ...
            if config.weights is not None:
                checkpoint_data = load_checkpoint(
                    config.weights, model=model, logger=logger, mode="emb"
                )
                model = (
                    checkpoint_data["model"] if "model" in checkpoint_data else model
                )

            model_info = {"model": model, "model_type": "torch"}
        else:
        ...
        embeddings, labels, file_names = evaluator.generate_embeddings(
            data_info_train, n_batches=config.n_batches
        )
    ...
    pca = PCA(n_components=config.n_components, whiten=getattr(config, "whiten", False))
    embeddings_new = pca.fit_transform(embeddings)
    print(embeddings_new.shape)

    pca_save_path = (
        work_dir / f"pca_from{config.embeddings_size}_to{config.n_components}.npz"
    )
